...
(defvar motd--git-commit-result nil)
(defun motd--git-commit ()
  ...
  (let* ((arg-string (format "cd %s && git add -A && git commit -m \"%s\""
    ...
    (async-cmd-start
     "sh" `("-c" ,arg-string)
     (lambda (id exit-status buffer)
       (setq motd--git-commit-result (with-current-buffer buffer (buffer-string)))
       (message "[motd-git] %s is committed." motd--git-commit-dir)))))

;;  (format "Auto Git Commit:\n%s\n"

(defvar motd--gitlab-commit-summary-result nil)
(defun motd--gitlab-commit-summary ()
  ...
  (async-cmd-start "python3" `(,(expand-file-name "~/.emacs.d/site-tools/GitlabMonitor/gitlab_monitor.py"))
                   (lambda (id exist-status buffer)
                     (setq motd--gitlab-commit-summary-result (with-current-buffer buffer (buffer-string))))))

(defvar motd--ccfddl-summary-result nil)
(defun motd--ccfddl-summary ()
  "Call ccfddl script asynchronously and append output to motd buffer."
  (async-cmd-start "python3" `(,(expand-file-name "~/.emacs.d/site-tools/CCFDDL/conference_deadlines.py"))
                   (lambda (id exist-status buffer)
                     (setq motd--ccfddl-summary-result (with-current-buffer buffer (buffer-string))))))
...
(defun motd--report-content-to-buffer ()
  ...
  (let ((ids (list
    ...
    (async-cmd-wait-and-cleanup
     ids
     (lambda (results) ;; (list id exist-status)
       (with-current-buffer motd--buffer
         (erase-buffer)
         (insert "<<Emacs Daily Report>>\n")
         (insert (format "\n%s\n" motd--ccfddl-summary-result))
         (insert (format "\n%s\n" motd--gitlab-commit-summary-result))
         (insert (format "\n%s\n" motd--git-commit-result))
         (goto-char (point-min)))
