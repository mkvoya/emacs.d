...
class GitLabMonitor:
    def __init__(self, config):
        # 从配置获取 GitLab 配置
        self.gitlab_url: str = config['gitlab']['url']
        self.gitlab_token: str = config['gitlab']['token']
    ...
    def get_user_commits(self, username: str, days: int = 3):
        """
        获取指定用户在最近几天内的提交记录
        Args:
        ...
        try:
            ...
            # 计算时间范围
            since_date = datetime.now().date() - timedelta(days)
